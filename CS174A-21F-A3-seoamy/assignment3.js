...
export class Assignment3 extends Scene {
    constructor() {
        ...
        this.materials = {
            ...
            planet2_phong: new Material(new defs.Phong_Shader(),
                { specularity: 1, diffusivity: 0.2, color: hex_color("#80FFFF") }),
            planet2_gourad: new Material(new Gouraud_Shader(),
                { specularity: 1, diffusivity: 0.2, color: hex_color("#80FFFF") }),
            ...
            planet4: new Material(new defs.Phong_Shader(),
                { smoothness: 100, specularity: 1, color: hex_color("#00BFFF") }),
            moon: new Material(new defs.Phong_Shader(),
                { color: hex_color("#FF1493") })
        }
    ...
    display(context, program_state) {
        ...
        var revolving_rate = 1;

        let sun_transform = Mat4.identity();
        var sun_scale = Math.sin(0.6 * t - (Math.PI / 2)) + 2;
        var sun_color = color(1, -0.5 + 0.5 * sun_scale, -0.5 + 0.5 * sun_scale, 1);
        ...
        planet1_transform = planet1_transform.times(Mat4.rotation(revolving_rate * t, 0, 1, 0))
        ...
        planet2_transform = planet2_transform.times(Mat4.rotation(revolving_rate * t, 0, 1, 0))
            .times(Mat4.translation(8, 0, 0))
            .times(Mat4.rotation(t, 0, 1, 0));
        this.planet_2 = planet2_transform;
        revolving_rate -= 0.15;

        let planet2_material = this.materials.planet2_phong;
        if (Math.floor(t) % 2 == 1) { // odd second --> gourad shading
            planet2_material = this.materials.planet2_gourad;
        }
        this.shapes.planet2.draw(context, program_state, planet2_transform, planet2_material);

        // planet 3
        let planet3_transform = Mat4.identity();
        planet3_transform = planet3_transform.times(Mat4.rotation(revolving_rate * t, 0, 1, 0))
            .times(Mat4.translation(11, 0, 0))
            .times(Mat4.rotation(revolving_rate * t, 0, 1, 0))
            .times(Mat4.translation(0, -1, 0))
            .times(Mat4.rotation(revolving_rate * t, 1, 0, 0))
            .times(Mat4.translation(0, 1, 0))
            .times(Mat4.rotation(revolving_rate * t, 0, 1, 0));
        planet3_transform = planet3_transform.times(Mat4.rotation(revolving_rate * t, 0, 1, 0));

        // planet 3 rings
        let rings_transform = planet3_transform.times(Mat4.scale(3, 3, 0.1));
        this.shapes.planet3.draw(context, program_state, planet3_transform, this.materials.planet3);
        this.shapes.torus.draw(context, program_state, rings_transform, this.materials.planet3_rings);
        ...
        planet4_transform = planet4_transform.times(Mat4.rotation(revolving_rate * t, 0, 1, 0))
            .times(Mat4.translation(14, 0, 0))
            .times(Mat4.rotation(revolving_rate * t, 0, 1, 0));
        let moon_transform = Mat4.identity().times(planet4_transform)
            .times(Mat4.rotation(revolving_rate * t, 0, 1, 0))
        ...
        if (this.attached != undefined) {
            // if camera is to view solar system, attach to intiial camera
...
class Ring_Shader extends Shader {
    ...
    fragment_glsl_code() {
        ...
        return this.shared_glsl_code() + `
        void main(){
            gl_FragColor = sin(50.0 * distance(center, point_position)) * vec4(0.57, 0.45, 0.25, 1);
        }`;
    }
}
